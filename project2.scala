...
object GossipSimulator{
  ...
  var gossipNodeCounter:Integer=0
  ...
  /*numberOfSensors=1728
  inputTopology="imperfect3d"
  inputAlgorithm="gossip"
  inputSumAvg="avg"*/
  
  gossipNodeCounter=numberOfSensors
...
class Sensor(value:Float,weight:Float) extends Actor{
  ...
  def receive={
        ...
        case gossipStringFromSensor(topology,algorithm,numNodes,sensorArrayBuffer)=>
          ...
          if(gossipCount==0){
          schedulor = context.system.scheduler.schedule( 0 millis, 1000 millis, self, gossipStringFromSensor(topology,algorithm,numNodes,sensorArrayBuffer))
          }
          ...
          if(gossipCount<10)
          {
            ...
            gossipNodeCounter=gossipNodeCounter-1
            if(gossipNodeCounter==0)
            {
              printThis("Milli Seconds terminated for topology to terminate "+((System.currentTimeMillis-startTime)))
            }
          }
